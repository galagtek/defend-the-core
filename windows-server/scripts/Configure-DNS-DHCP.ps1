...
param(
    ...
    [Parameter(Mandatory=$false)]
    [string]$DhcpScopeEnd = "10.10.50.200",
...
try {
    Add-DhcpServerInDC -DnsName $dhcpFqdn -IPAddress "10.10.50.20" -ErrorAction SilentlyContinue
...
Set-DhcpServerv4OptionValue -ScopeId $scopeId -Router $DhcpRouter `
    -DnsServer "10.10.50.20" -DnsDomain $DomainName -ErrorAction SilentlyContinue
Write-Log "Options DHCP configurees :"
Write-Log "  - Routeur (passerelle) : $DhcpRouter"
Write-Log "  - Serveur DNS : 10.10.50.20"
...
Write-Log "  DHCP scope : 10.10.30.100 - 10.10.50.200 (/24)"
